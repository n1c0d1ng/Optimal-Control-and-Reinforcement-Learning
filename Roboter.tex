...
Die Richtungsableitung des Funktionals $J$ an der Stelle $x$ in Richtung $h$ ist definiert als 
%
\begin{align*}
\delta J(x(t), h(t)) = \lim_{\epsilon \to 0} \frac{J[x(t)+ \epsilon \cdot h(t)] - J[x(t)]}{\epsilon} =
\gruen{\frac{d}{d\epsilon} J[x + \epsilon h] \bigg|_{\epsilon = 0}}
\end{align*}
\end{definition}

Wir definieren $f:\mathbb{R} \to \mathbb{R}$ mit $f(\epsilon) := J[x+ \epsilon \cdot h]$, dann gilt
%
\begin{align*}
\gruen{f'(0)} = \lim_{\epsilon \to 0} \frac{f(\epsilon) -f(0)}{\epsilon} = \delta J(x(t), h(t))
\end{align*}

%
%---------------------------------------------------------------------------------------------------------
\begin{satz}[Euler-Lagrange-Gleichung]  \label{ELE}
Sei $J: C^{2}([a,b]) \to \mathbb{R}$ ein Funktional mit
%
\begin{align*}
J[x] = \int_{t_0}^{t_f} L(t,x(t),x'(t)) \mathrm{d}t, \quad x(t_0) = x_0, x(t_f) = x_f
\end{align*}
%
Dann gilt für die Ableitung $\nabla J(x) = L_x -  \frac{\mathrm{d}}{\mathrm{d}t} L_{x'}$. Sei $x^* (t) = \textsf{argmin } J[x] $
erfüllt es die folgende Gleichung
%
\begin{align*}
\nabla J(x^*) = 0 \Leftrightarrow L_{x} - \frac{d}{dt} L_{x'} = 0
\end{align*}
\end{satz}
...
%
\begin{align*}
\frac{d}{d\epsilon} J[x + \epsilon h] \bigg|_{\epsilon = 0} 
&= \int_{t_0}^{t_f} \frac{d}{d\epsilon} L(t, x(t) + \epsilon h(t), x'(t) + \epsilon h'(t)) \dt \bigg|_{\epsilon = 0} \\
&= \int_{t_0}^{t_f} L_x \cdot h(t) + \orange{L_{x'} \cdot h'(t)} \dt \\
&= \int_{t_0}^{t_f} L_x \cdot h(t)  \orange{ - \frac{d}{dt}  L_{x'} \cdot h(t)} \dt + \orange{\left[ L_{x'} \cdot h(t) \vphantom{\int}  \right]_{t_0}^{t_f} }
\end{align*}
%
Für den Gradienten gilt $\delta J(x(t), \dunkelrot{h(t)}) = \gruen{\nabla J(x)} \cdot \dunkelrot{h(t)}$
%
\begin{align*}
\delta J(x(t), \dunkelrot{h(t)}) = \int_{t_0}^{t_f} \left( \gruen{L_{x} - \frac{d}{dt} L_{x'} } \right) \cdot \dunkelrot{h(t)} \dt + \left[ \gruen{L_{x'}} \cdot \dunkelrot{h(t)} \vphantom{\int}  \right]_{t_0}^{t_f}
\end{align*}
%
Für feste Randwerte $x(t_0) = x_0, x(t_f) = x_f$ gilt $h(t_0) = h(t_f) = 0$.
}
...
Wir betrachten unser Variationsproblem, dann definieren wir den adjungierten Zustand $p$ und die Hamilton-Funktion $H$ durch
%
\begin{align*}
p(t) := L_{x'} \quad H(t,x,p) := x'(t) \cdot p(t) - L(t,x,x')  
\end{align*}

Die Hamilton-Funktion besitzt wird somit extremal entlang $x'$
%
\begin{align*}
H_{x'} =  p - L_{x'} = 0
\end{align*}
...
\begin{itemize}
\item Partiell Differenzieren von $H$ bezüglich $x$ und $p$ liefert
%
\begin{align*}
\frac{\partial H}{\partial p} &= x' + p \cdot \frac{\partial x'}{\partial p} - \underbrace{L_{x'}}_{= p} \cdot \frac{\partial x'}{\partial p} = x'(t) \\
\frac{\partial H}{\partial x} &= - L_{x} = -\frac{\mathrm{d}}{\dt} L_{x'} =  -p'(t)
\end{align*}
%
\item Im autonomen Fall $H_{t} = 0$ gilt: $\orange{H_x = - p'}$ und $\lila{H_p = x'}$
%
\begin{align*}
\frac{\mathrm{d}H}{\mathrm{d}t} = H_t + \orange{H_x} \cdot x'(t) + \lila{H_p} \cdot  p'(t) = H_t = 0
\end{align*}
\end{itemize}
...
Sei $x^{*} (t)$ ein Minimum eines Variationsproblems, so gilt:
%
\begin{itemize}
\item Die Funktion $x$ erfüllt die Euler-Lagrange Gleichung.
%
\begin{align*}
\nabla J(x^*) = 0 \Leftrightarrow L_{x} - \frac{\mathrm{d}}{\dt} L_{x'} = 0
\end{align*}

\item Für eine freie Endzeit $\dunkelgelb{t_f}$ gilt 
%
\begin{align*}
 H(\dunkelgelb{t_f},x,p) = p(\dunkelgelb{t_f}) \cdot x'(\dunkelgelb{t_f}) - L(\dunkelgelb{t_f})  = 0
\end{align*}

\item Für einen freien Endpunkt $x_{f}$ gilt
%
\begin{align*}
 p(t_{f}) = L_{x'}(t_{f})  = 0
\end{align*}
...
\proof{
Für festes $x(t_0) = x_0$ gilt $\dunkelrot{h(t_0)} = 0$. Bei freiem $x(t_f) = x_f$ verschwindet der Randterm für $\gruen{L_{x'}(t_f) }= 0$ und es gilt
%
\begin{align*}
\delta J(x(t), \dunkelrot{h(t)}) 
&= \int_{t_0}^{t_f} \left( \underbrace{\gruen{L_{x} - \frac{d}{dt} L_{x'} }}_{ = 0} \right) \cdot \dunkelrot{h(t)} \dt + \left[ \gruen{L_{x'}} \cdot \dunkelrot{h(t)} \vphantom{\int}  \right]_{t_0}^{t_f} \\
0 &=\underbrace{ \gruen{L_{x'}(t_f)}}_{= 0} \cdot \dunkelrot{h(t_f)} - \gruen{L_{x'}(t_0)} \cdot \underbrace{\dunkelrot{h(t_0)}}_{= 0} 
\end{align*}
...
\begin{align*}
    M(q)\,\ddot{q} + C(q,\dot{q})\,\dot{q} + g(q) = \tau + d(q,\dot{q})
\end{align*}

mit Störung/Unsicherheit $d$. Zustand:
...
Direkte Eingabe $\tau$: kanonische Zustandsdarstellung -- explizit nach $\ddot{q}$ auflösen:
\begin{align*}
    \ddot{q} &= M(q)^{-1}\big(\tau - C(q,\dot{q})\,\dot{q} - g(q) + d(q, \dot{q}) \big)
\end{align*}
...
\begin{align*}
    ...
    \underbrace{
        \begin{bmatrix}
            \dot{q}\\[2pt]
            -\,M(q)^{-1}\big(C(q,\dot{q})\,\dot{q} + g(q)\big)
        \end{bmatrix}}_{A(x)}
    +
    \underbrace{
        \begin{bmatrix}
...
Merke: In dieser "reinen" Form ist $B(x)$ die Eingabematrix zur physischen Eingabe $\tau$. 
Dein gewünschtes Schema ist dann wörtlich
\begin{align*}
    x' = A(x) + B(x) \tau + \Delta(x)
\end{align*}

LinReparametrisierung: nominales Modell + strukturierte Unsicherheit. Lin führt eine Nominaldynamik ein, 
um Unsicherheit strukturiert zu kapseln.
Nominalgrößen wählen: $M_0(q),\; N_0(q,\dot{q}):=C_0(q,\dot{q}) \dot{q} + g_0(q)$
Neue Eingabe definieren:

\begin{align*}
    u := M_0(q)^{-1} \big( \tau - N_0 (q,\dot{q}) \big)
\end{align*}
...
\begin{align*}
    ...
    M(q)^{-1}M_0(q)\,u
    +
    M(q)^{-1}\big(N_0(q,\dot{q})-N(q,\dot{q})\big)- Strukturterme:
    h(x) := M(q)^{-1}M_0(q) - I,\qquad
    f(x) := M(q)^{-1}\big(N_0(q,\dot{q})-N(q,\dot{q})\big):
    \dot{x}
    =
...
Interpretation: Das nominale Hilfssystem ist DoppelIntegrator in $(q,\dot{q})$, 
und alle Nichtlinearitäten/Parameterfehler erscheinen als
matched Unsicherheit $h(x)u$ (EingabematrixUnsicherheit, mit $h(x)\succeq 0$ in Lins Annahmen),
additive matched Störung $f(x)$ (wirkt über dieselbe Richtung wie $u$).
Diese Struktur erlaubt Lins Hilfsproblem und die Reduktion auf LQR, sobald du lokal linearisierst 
und eine quadratische Schranke $\|f(x)\|^2 \le x^\top P x$ aufstellst.
Häufige Stolperstelle in der Notation- In „$x' = A(x) + B(x) + \Delta_x$“ fehlt meist das $u$. 
Korrekt ist $x' = A(x) + B(x)\,u + \Delta(x)$.
Falls du $u$ bereits in $\Delta$ „eingebaut“ hast (LinForm), dann schreib klar: 
\begin{align*}
    \Delta(x,u) = [0;\,h(x)u + f(x)]
\end{align*}

Kurzformeln zum direkten Einsetzen- Mit physischer Eingabe $\tau$:
...
\begin{align*}
    ...
    0\\
    M^{-1} d
    \end{bmatrix}.- Mit Lin‑Eingabe u = M_0^{-1}(\tau - N_0):
    A(x)=
    \begin{bmatrix}
    \dot{q}\\
    0
...
\subsection{Mein Ansatz auf Lin}
% -------------------------
% Variante A: Direkte Zustandsform mit physischer Eingabe \tau
% -------------------------
% Zustandsvektor
\begin{align*}
...
% EL‑Dynamik mit Störung d(q,\dot q)
\begin{align*}
    M(q)\,\ddot{q} + C(q,\dot{q})\,\dot{q} + g(q) = \tau + d(q,\dot{q})
\end{align*}

% Explizit nach \ddot q aufgelöst und Zustandsform
\begin{align*}
    \ddot{q} = M(q)^{-1}\big(\tau - C(q,\dot{q})\,\dot{q} - g(q) + d(q,\dot{q})\big).
    \Rightarrow
    \dot{x} =
    \underbrace{\begin{bmatrix} \dot{q} \\[4pt] -M(q)^{-1}\big(C(q,\dot{q})\,\dot{q} + g(q)\big) \end{bmatrix}}_{A(x)}
    +
    \underbrace{\begin{bmatrix} 0 \\[4pt] M(q)^{-1} \end{bmatrix}}_{B(x)}\,\tau
    +
    \underbrace{\begin{bmatrix} 0 \\[4pt] M(q)^{-1} d(q,\dot{q}) \end{bmatrix}}_{\Delta(x)}
\end{align*}
