...
\begin{align*}
    ...
    \bm{a}^{(1)}
    \\
    \frac{\partial V}{\partial \bm{z}_k^{(1)}}
    &=
    \frac{\partial V}{\partial \bm{a}_k^{(1)}}
    \cdot 
    \frac{\partial \bm{a}_k^{(1)}}{\partial \bm{z}_k^{(1)}}
    =
    w_{k}^{(N)} \cdot
    \left[
        1 - \left( \bm{a}^{(1)}_k \right)^2
    \right]
    \Rightarrow
    ...
    \bm{\delta}^{(1)}
    \\
    \frac{\partial V}{\partial w_{ij}^{(1)}}
    &=
    \frac{\partial V}{\partial \bm{z}_{i}^{(1)}} \cdot 
    \frac{\partial \bm{z}_i^{(1)}}{\partial w_{ij}^{(1)}}
    =
    \begin{cases}
        \bm{\delta}_i^{(1)} \cdot t , & j=1 \\
        \bm{\delta}_i^{(1)} \cdot x , & j=2
    \end{cases}
    \Rightarrow
    ...
    \end{bmatrix}
    \\
    \frac{\partial V}{\partial \bm{b}_i^{(1)}}
    &=
    \frac{\partial V}{\partial \bm{z}_i^{(1)}} \cdot 
    \frac{\partial \bm{z}_i^{(1)}}{\partial \bm{b}_i^{(1)}}
    =
    \bm{\delta}_i^{(1)} \cdot 1 
    \Rightarrow
    \frac{\partial V}{\partial \bm{b}^{(1)}} = \bm{\delta}^{(1)}
\end{align*}
...
\begin{satz}[Neuronal-Networks]
    Sei $V:\mathbb{R}^{\text{Input}} \to \mathbb{R}^{\text{Output}}$ approximiert durch $V \approx \bm{z}_N$
    \begin{itemize}
        \item  Forwardpass:
        %
        \begin{align*}
            \bm{z}^{(n)} &= \bm{W}^{(n)} \cdot \bm{a}^{(n-1)} + \bm{b}^{(n)} \quad n=1,\cdots , L  \\
            \bm{a}^{(0)}  &= \bm{x}_{\text{Input}} \\
            \bm{a}^{(n)}  &= \tanh\left( \bm{z}^{(n)} \right) \\
            \bm{z}^{(N)} &= \bm{W}^{N} \cdot \bm{a}^{(L)} + \bm{b}^{(N)}
        \end{align*}
        %
        ...
        \begin{itemize}
            \item[1.] Output-Layer
            \begin{align*}
                \frac{\partial V}{\partial \bm{W}^{(N)}} = \bm{a}^{(L)} 
                \quad
            ...
            \item[2.] Für jede Schicht $k= L, L-1 \cdots , 1$
            %
            \begin{align*}
                ...
                \mathsf{diag}
                \left[
                    1 - \left( \bm{a}^{(k)}_i \right)
                \right]_{i=1, \cdots , n_k}
                :=
                \bm{\delta}^{(k)} 
                \\
                ...
                &=
                \left(\bm{\delta}^{(k)}\right)^T
                \\
                \frac{\partial V}{\partial \bm{a}^{(k-1)}}
                &=
                \bm{\delta}^{(k)} \cdot \bm{W}^{(k)}, \quad k > 1
            \end{align*}
        \end{itemize}
    \end{itemize}
\end{satz}
...
\proof{
    Wir skizzieren den Beweis für $L=2$. 
    \begin{itemize}
        \item[1.] Ableitung Output-Layer $V = \bm{W}^{(N)} \cdot \bm{a}^{(2)} + b^{(N)}$:
        %
        \begin{align*}
            \frac{\partial V}{\partial w_i^{(N)}} =
            \bm{a}_i^{(2)}
            \quad
            \frac{\partial V}{\partial b_N} = 1
            \quad
            \frac{\partial V}{\partial \bm{a}_k^{(2)}} = \bm{W}_k^{(N)} 
        \end{align*}

        \item[2.] Ableitung durch die Aktivierungsfunktion: Im Vektorfall $\bm{a}_k = \tanh(\bm{z}_k)$ 
        gilt für jede Komponente $i$
        %
        \begin{align*}
        ...
        %
        \begin{align*}
            \frac{\partial \bm{a_k}}{\partial \bm{z_k}} 
            =
            \left[
                \frac{\partial \tanh\left( \bm{z}_i^{(k)} \right)}{\partial \bm{z}_j^{(k)}}
        ...
        \item[3.] Für die Ableitung der Hidden-Layer Struktur gilt mittels Kettenregel:
        %
        \begin{align*}
            \frac{\partial V}{\partial \bm{z}_k^{(2)}} 
            &= \frac{\partial V}{\partial \bm{a}_k^{(2)}} 
            \cdot \frac{\partial \bm{a}_k^{(2)}}{\partial \bm{z}_k^{(2)}}
            =
            \bm{W}_k^{(N)} \cdot \left[1 - \left(\bm{a}_k^{(2)} \right)^2 \right]
            \\
            \bm{\delta}^{(2)} 
            :=
            \frac{\partial V}{\partial \bm{z_2}} 
            &=
            \bm{W}_N \cdot 
            \mathsf{diag}\left[1- \left(\bm{a}_i^{(2)}\right)^2 \right]_{i=1,\cdots , n_2} \in \mathbb{R}^{1 \times n_2}
        \end{align*}

        \item[4.] Parameter von Hidden-Layer $2$:
        %
        \begin{align*}
            \frac{\partial V}{\partial w_{ki}^{(2)}}
            &=
            \frac{\partial V}{\partial \bm{z}_k^{(2)}} 
            \cdot
            \frac{\partial \bm{z}_k^{(2)}}{\partial w_{ki}^{(2)}}
            =
            \bm{\delta}^{(2)}_k \cdot \bm{a}_i^{(1)}
            \Rightarrow
            \frac{\partial V}{\partial \bm{W}^{(2)}}
            =
            \left(\bm{\delta}^{(2)} \right)^T \cdot \left(\bm{a}^{(1)}\right)^{T}
            \\
            \frac{\partial V}{\partial \bm{b}_k^{(2)}}
            &=
            \frac{\partial V}{\partial \bm{z}_k^{(2)}} \cdot
            \frac{\partial \bm{z}_k^{(2)}}{\partial \bm{b}_k^{(2)}}
            =
            \bm{\delta}^{(2)}_k \cdot 1
            \Rightarrow
            \frac{\partial V}{\partial \bm{b}^{(2)}}
            =
            \bm{\delta}^{(2)}
        \end{align*}

        \item[5.] Jede Komponente von $\bm{z}^{(2)}$ ist abhängig von $\bm{a}^{(1)}$, denn
        %
        \begin{align*}
            \bm{z}^{(2)} 
            =
            ...
            =
            \begin{bmatrix}
                \sum_{k=1}^{n_1} w_{1k}^{(2)} \cdot \bm{a}_{k}^{(1)} + \bm{b}_1^{(2)} \\
                \sum_{k=1}^{n_1} w_{2k}^{(2)} \cdot \bm{a}_{k}^{(1)} + \bm{b}_2^{(2)} \\
                \vdots \\
                \sum_{k=1}^{n_1} w_{n_{2} k}^{(2)} \cdot \bm{a}_{k}^{(1)} + \bm{b}_{n_2}^{(2)}
            \end{bmatrix}
        \end{align*}

        \item[6.] Mittels der Kettenregel folgt somit:
        %
        \begin{align*}
            \frac{\partial V}{\partial \bm{a}_k^{(1)}} 
            &=
            \sum_{i=1}^{n_2} \frac{\partial V}{\partial \bm{z}_i^{(2)}}
            \cdot
            \frac{\partial \bm{z}_i^{(2)}}{\partial \bm{a}_k^{(1)}}
            =
            \sum_{i=1}^{n_2} \bm{\delta}_i^{(2)} \cdot w^{(2)}_{ik}
            \Rightarrow
            \frac{\partial V}{\partial \bm{a}^{(1)}}
            =
            \bm{\delta}^{(2)} \cdot \bm{W}^{(2)} \in \mathbb{R}^{1 \times n_1}
            \\
            \frac{\partial V}{\partial \bm{z}_k^{(1)}} 
            &=
            \frac{\partial V}{\partial \bm{a}_k^{(1)}} \cdot 
            \frac{\partial \bm{a}_k^{(1)}}{\partial \bm{z}_k^{(1)}}
            =
            \sum_{i=1}^{n_2} \bm{\delta}_i^{(2)} \cdot w^{(2)}_{ik}
            \cdot
            \left[
                1 - \left(\bm{a}_k^{(1)}\right)^2 
            \right]
        \end{align*}
        ...
        \begin{align*}
            ...
            =
            \left(
                \bm{\delta}^{(2)} \cdot \bm{W}^{(2)}
            \right) 
            \cdot 
            \mathsf{diag}
            \left[
                1 - \left( \bm{a}_i^{(1)} \right)^2
            \right]_{i=1,\cdots, n_1}
            \in \mathbb{R}^{1 \times n_1}
        \end{align*}

        \item[7.] Ableitung nach den Parametern in Hidden-Layer Schicht $1$:
        %
        \begin{align*}
            \frac{\partial V}{\partial w^{(1)}_{ij}} =
            \frac{\partial V}{\partial \bm{z}^{(1)}_{i}} \cdot 
            \frac{\partial \bm{z}^{(1)}_{i}}{\partial w^{(1)}_{ij}}
            =
            \begin{cases}
                \bm{\delta}_i^{(1)} \cdot t , & j=1 \\
                \bm{\delta}_i^{(1)} \cdot x , & j=2 \\
