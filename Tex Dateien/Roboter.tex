...
\begin{satz}[Optimality Equation für $V$]
...
\begin{satz}[Optimality Equation für Q-Funktion]
...
Für die Wahrscheinlichkeit jeder Trajektorie gilt
\begin{align*}
    P(\tau_0) &= 
    P(\bm{s}_0) \cdot 
    \prod_{t=0}^{\infty} \pi(a_t|\bm{s}_t) \cdot T_{a_t}(\bm{s}_t, \bm{s}_{t+1})
\end{align*}

Wir folgen der Notation aus \cite{sutton_barto_2018} und definieren: 
%
...
\begin{definition}[Objective Function]
    ...
    \begin{align*}
        ...
        V^{\pi}(\bm{s}_0)
        =
        \mathsf{E}_{\tau_0 \sim P(\tau_0 \mid \pi)} 
        \left[
            \vphantom{\int}
            R(\tau_0)
        \right]
...
Wir definieren die Abbildung $\mathcal{F}$ als
\begin{align*}
    \mathcal{F}[Q](\bm{s}, a)
    &=
    \mathsf{E}_{\bm{s}' \sim T_a(\bm{s})}
...
Die Bellmanngleichung sagt nun aus, 
dass $Q^*$ der Fixpunkt der Abbildung $\mathcal{F}$ ist, also
%
\begin{align*}
    Q_{k+1} = \mathcal{F}[Q_k]
\end{align*}
...
\State $V_{k+1}(s) \gets \max_{a} \mathsf{E}_{s' \sim T_a(s)} 
\left[ 
    \vphantom{\int}
    R_a(s, s') + \gamma V_k(s') 
    \right]$
