...
\proof{
    \begin{itemize}
        %
        \item Es gilt $\pi_\epsilon = \pi + \epsilon \cdot h$
        %
        \begin{align*}
        ...
        \end{align*}
        %
        \item Für jeden festen Zeitschritt $t$ gilt somit:
        %
        \begin{align*}
            \mathsf{E}_{\tau \sim p(\tau|\pi)}
            \left[
            ...
            \left[
                \lila{\int_{\mathcal{A}}}
                \lila{\pi(a_t = a \mid \bm{s}_t)} \cdot
                \frac{h(a_t|\bm{s}_t)}{\pi(a_t = a| \bm{s}_t)}
                \cdot
                Q^{\pi}(\bm{s}_t,a_t = a)
                \lila{\mathrm{d}a}
            \right]
        ...
        \end{align*}
        %
        \item Wir erhalten somit insgesamt:
        %
        \begin{align*}
            \frac{d}{d \epsilon} J(\pi_\epsilon) \mid_{\epsilon=0}
            &=
            \sum_{t=0}^{T-1}
            \mathsf{E}_{\blau{\bm{s}_t}}
            \left[
                \int_{\mathcal{A}}
                h(a|\bm{s}_t) \cdot
                Q^{\pi}(\bm{s}_t,a)
            ...
            \\
            &=
            \blau{\int_{\mathcal{S}}} \int_{\mathcal{A}}
            \sum_{t=0}^{T-1}
            p(\bm{s}_t = \bm{s}) \cdot
            h(a |\bm{s}) \cdot
            Q^{\pi}(\bm{s},a)
        ...
        \begin{align*}
            ...
            \underbrace{
                ...
                p_{\theta}(\bm{s}_t = \bm{s},a_t = a)
            }
            \cdot \nabla_{\theta} \log \pi_{\theta}(a|\bm{s})
            \mathrm{d}a \; \mathrm{d}\bm{s} \\
            &=
            \sum_{t=0}^{T-1}
            \mathsf{E}_{(\bm{s}_t, a_t) \sim p_{\theta}}
                \left[
                    \vphantom{\int}
                    \mathsf{E}_{\tau \sim p(\tau|\theta)}
                    \left[
            ...
            \sum_{t=0}^{T-1}
            \underbrace{
                \mathsf{E}_{(\rot{\bm{s}_t, a_t}) \sim p_{\theta}}
                \left[
                    \vphantom{\int}
                    \mathsf{E}_{\tau \sim p(\tau|\theta)}
                    \left[
                        R(\tau) 
                        \cdot
                        \nabla_{\theta} \log \pi_{\theta}(a_t|\bm{s}_t) \mid \rot{\bm{s}_t, a_t}
                    \right]
                \right]
            }_{
                \mathsf{E}_{\tau \sim p(\tau|\theta)}
                ...
                \right],
                \text{ Tower Property: }
                \mathsf{E}[X] = \mathsf{E}_{\rot{Y}}[\mathsf{E}[X|\rot{Y}]]
            }
                \\
            &=
            \mathsf{E}_{\tau \sim p(\tau|\theta)} 
    ...
    \begin{algorithmic}
        ...
        \While{$\|\nabla_{\theta} J(\pi_{\theta})\|_2 > \epsilon$}
            ...
            \For{$t \in 0, \cdots , T-1$}
                ...
                R_t \cdot \nabla_{\theta} \log \pi_{\theta}(a_t|\bm{s}_t)$
                \Comment{Wir zerlegen die Summe
                $\sum_{t=0}^{T-1} Q^{\pi_{\theta}}(\bm{s}_t, a_t) 
                \nabla_{\theta} \log \pi_{\theta}(a_t|\bm{s}_t)
                \approx
                $}
            \EndFor
